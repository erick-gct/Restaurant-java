...
public class Controller
{
    ...
    private void selectMenu()
    {
         ...
         while( selection == 0 && this.state == 0)
         {
            try
            {
                printMessageToView("Por favor seleecione uno:");
                String key = cView.userInput();
                
                if(key.equalsIgnoreCase("Q"))   //quit program
                {
                    printMessageToView("Está seguro que desea salir? (Y:YES)");
                    key = cView.userInput();
                ...
                {
                    printMessageToView("Seguro que quieres marcar la salida? (Y:YES)");
                    key = cView.userInput();
                    if (key.equalsIgnoreCase("Y")) {
                        Staff rStaff = cDatabase.findStaffByID(currentUserID);
                        if(rStaff.getWorkState() == Staff.WORKSTATE_FINISH)
                        {
                            printMessageToView("Marcaste salida!");
                        }
                        else
                        {
                            rStaff.clockOut();
                            printMessageToView("Gracias por tu trabajo!!");
                        }
                    }
                    pause(3);
                    //reflesh view
    ...
    {
        cView.loginView();
        printMessageToView("Login como manager? (Y/N)");
    ...
    private void loginCheck( boolean isManager)
    {
        ...
        printMessageToView("Entra tu ID.");
        while(inputID == 0)
        {
            key = cView.userInput();
            try{
            ...
            {
                printMessageToView("Solamente números son aceptados.\nEnter your ID.");
            }
        }
        printMessageToView("Ingresa la contraseña.");
        ...
        if( rStaff != null)//User data is found
        {
            ...
            if( rStaff.getClass().getName().equalsIgnoreCase(searchClassName))
            {
                ...
                {
                    printMessageToView("Login existoso!!");
    ...
    public void addNewOrderItem(int orderID)
    {
        ...
        while(!done)
        {
            ...
            printMessageToView("Añadir algun otro item?(yes:y)");
            key = cView.userInput();
    ...
    public void deleteOrderItem(int orderID)
    {
        ...
        if( currentUserID != rOrder.getStaffID())
        {
            printErrorMessageToView("No tienes permitidio eliminar la orden.");
            printMessageToView("(La orden pertence a  " + rOrder.getStaffName() + ")");
            pause(2);
            return;    
        }
